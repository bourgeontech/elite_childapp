package com.burgeon.parentapp.Utils;

import android.content.Context;
import android.content.SharedPreferences;

/**
 * Created by Ajay K K on 2019-11-11.
 */
public class PreferenceRequestHelper {


    private static PreferenceRequestHelper mSharedPreferenceUtils;
    protected Context mContext;
    private SharedPreferences mSharedPreferences;
    private SharedPreferences.Editor mSharedPreferencesEditor;

    public PreferenceRequestHelper(Context context) {
        mContext = context;
        mSharedPreferences = context.getSharedPreferences("com.school.android", Context.MODE_PRIVATE);
        mSharedPreferencesEditor = mSharedPreferences.edit();
    }

    public static synchronized PreferenceRequestHelper getInstance(Context context) {
        if (mSharedPreferenceUtils == null) {
            mSharedPreferenceUtils = new PreferenceRequestHelper(context.getApplicationContext());
        }
