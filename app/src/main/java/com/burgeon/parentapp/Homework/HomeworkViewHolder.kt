...
import kotlinx.android.synthetic.main.my_remark.view.*
...
class HomeworkViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(news: HomeworkDetailsmodel?) {
        if (news != null) {
            ...
            itemView.llUploaddoc.setOnClickListener {
                try {
                    val discussionList =
                        uploadDocFragment.newInstance(news?.id!!, "homework")
            ...
            itemView.llViewdoc.setOnClickListener {
                try {
                    val uloadeddoc =
                        UploadeddocFragment.newInstance(news?.id!!, "0")
            ...
            itemView.llRemark.setOnClickListener {
                if(news.remarks!=null) {
                    if (news.remarks?.length!! > 0)
                        MyDialog3(news?.remarks!!)
                }
            }
    ...
    private fun MyDialog3(remarks: String) {
        val mDialogView = LayoutInflater.from(contextval).inflate(R.layout.my_remark, null)
        //AlertDialogBuilder
        ...
        mDialogView.tvRemark.setText(remarks)
